...
| [`tracking`](#tracking) | `github.com/pickpoint/go-sdk/tracking` | Live GPS over WebSocket (`tracking.v2`) |
...
Live GPS is a **separate** WebSocket session: `wss://tracking.pickpoint.io/v2/ws`, subprotocol `tracking.v2`. It is not the HTTP `pickpoint.Client`.
...
session, err := tracking.Connect(ctx, tracking.Config{
	...
	Device: &tracking.DeviceAuth{ // from devices.Create — not the HTTP API key
		ClientID:     deviceUID,
...
defer session.Close() // TrackStop + hang up

session.Publish(&tracking.LatLng{Latitude: 55.75, Longitude: 37.61}) // TrackStart if idle
```

### Listener (dashboard)

The JWT is the **client-token** `access_token` — same one as HTTP `ClientAuth`. Mint it on your backend with scope `devices` (API key never goes in the dashboard).
...
session, err := tracking.Connect(ctx, tracking.Config{
	...
	Listener:  &tracking.ListenerAuth{AccessToken: pair.AccessToken},
	Subscribe: []string{deviceUID},
})
if err != nil {
	log.Fatal(err)
}
...
for {
	...
	if loc := msg.Loc; loc != nil { // live fan-out; publisher never sees Loc
		log.Printf("%f %f", loc.Point.Latitude, loc.Point.Longitude)
	}
}
```
